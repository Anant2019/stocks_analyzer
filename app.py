...
st.title("🛡️ 90% Accuracy Tracker (Final Fix)")
...
def run_backtest():
    ...
    # Target date se 90 din baad tak ka data check karenge targets ke liye
    fetch_end = target_date + timedelta(days=90)
    
    prog = st.progress(0)
    for i, ticker in enumerate(NIFTY_200):
        try:
            # Fixing the data fetch end date logic
            data = yf.download(ticker, start=target_date - timedelta(days=400), end=fetch_end, auto_adjust=True, progress=False)
            ...
            if close > sma44 and sma44 > sma200 and close > open_p:
                # 90% Probability Logic
                is_blue = rsi > 65 and vol > vol_avg and (close > sma200 * 1.05)
                risk = close - low_p
                t2 = close + (2 * risk)
                future = data[data.index > t_ts]
                
                status, hit, why = "⏳ Running", False, "Price trend positive hai."
                
                if not future.empty:
                    for _, f_row in future.iterrows():
                        if f_row['Low'] <= low_p:
                            status, why = "🔴 SL Hit", f"Support level toota. RSI {round(rsi,1)} ke baad bhi bears heavy rahe."
                            break
                        if f_row['High'] >= t2:
                            status, hit, why = "🔥 Jackpot Hit", True, f"Triple Bullish trend worked! RSI {round(rsi,1)} aur Volume boost ne 1:2 target dilaya."
                            break
...
if st.button('🚀 Run Analysis'):
    ...
    if not df.empty:
        ...
        # ANALYSIS ON CLICK
        st.divider()
        st.subheader("💡 Analysis on Click")
        selected = st.selectbox("Stock select karein 'Why' dekhne ke liye:", df["Stock"].tolist())
        if selected:
            s_data = df[df["Stock"] == selected].iloc[0]
            st.info(f"**{selected} Analysis:**")
            st.write(s_data["Analysis"])
    else:
        st.error("No signals found. Try another date (e.g., 2025-12-12).")
