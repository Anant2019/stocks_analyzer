...
st.set_page_config(
    page_title="ArthaSutra | Precision Support Audit", 
    layout="wide", 
...
st.markdown("""
    ...
    .trade-card {
        ...
        padding: 18px; margin-bottom: 5px; border-left: 5px solid #333;
    }
    .duration-badge {
...
with st.expander("📝 IMPORTANT: SEBI Non-Registration & Risk Warning", expanded=True):
    ...
    <div style="background-color: rgba(255, 193, 7, 0.05); padding:15px; border-radius:12px; border:1px solid rgba(255, 193, 7, 0.3);">
        ...
        <p style="color:#CCCCCC; font-size:0.95em;">
            <b>ArthaSutra</b> is a research engine. We are <b>NOT SEBI REGISTERED</b>. 
...
def run_arthasutra_engine(target_date):
    ...
    for i, ticker in enumerate(NIFTY_200):
        try:
            ...
            # --- STRICT SUPPORT LOGIC ---
            is_bullish_trend = d['SMA_44'] > d['SMA_200'] and d['SMA_200'] > prev_d['SMA_200']
            is_green_candle = d['Close'] > d['Open']
            
            # Distance from 44 SMA (Candle Low must be near or below SMA, or Close must be within 1% of SMA)
            dist_to_44 = abs(d['Close'] - d['SMA_44']) / d['SMA_44']
            is_at_support = d['Low'] <= d['SMA_44'] * 1.005 or dist_to_44 <= 0.01

            if is_bullish_trend and is_green_candle and is_at_support:
                is_blue = d['RSI'] > 60 and d['Volume'] > d['Vol_MA']
                ...
                t1_pct, t2_pct = round(((t1/d['Close'])-1)*100, 1), round(((t2/d['Close'])-1)*100, 1)
                ...
                if not future.empty:
                    for count, (f_dt, f_row) in enumerate(future.iterrows(), 1):
                        ...
                        if f_row['High'] >= t2: status = "🟢 Jackpot Hit"; t2_hit = True; days = count; break
                        if f_row['Low'] <= sl: status = "🔴 SL Hit"; days = count; break

                results.append({
                    "Stock": ticker.replace(".NS",""), "Cat": "🔵 BLUE" if is_blue else "🟡 AMBER",
                    "Status": status, "Entry": round(d['Close'], 2), "SL": sl, "T1": t1, "T2": t2,
                    "T1_Pct": t1_pct, "T2_Pct": t2_pct, "T1_Hit": t1_hit, "T2_Hit": t2_hit,
                    "Days": days, "RSI": round(d['RSI'], 1), "Chart": f"https://www.tradingview.com/chart/?symbol=NSE:{ticker.replace('.NS','')}"
                })
        except: continue
...
if st.button('🚀 Execute Support Strategy Audit', use_container_width=True):
    ...
    if not df.empty:
        ...
        m3.metric("📈 Total Support Signals", len(df))
        ...
        for _, row in df.iterrows():
            ...
            <div class="trade-card" style="border-left-color: {border_clr};">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span style="font-size: 1.3rem; font-weight: bold; color: white;">{row['Stock']} ({row['Cat']})</span>
                ...
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; margin-top: 15px; background: rgba(255,255,255,0.03); padding: 12px; border-radius: 8px;">
                    ...
                    <div><div class="rr-label" style="color:#00FFA3;">T1 (1:1)</div><div class="rr-value">₹{row['T1']}</div><div class="profit-pct">+{row['T1_Pct']}%</div></div>
                    <div><div class="rr-label" style="color:#00FFA3;">T2 (1:2)</div><div class="rr-value">₹{row['T2']}</div><div class="profit-pct">+{row['T2_Pct']}%</div></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            with st.expander(f"Analysis Rule #1: Support Audit for {row['Stock']}"):
                st.write(f"Trend: 200 SMA Bullish & 44 SMA Support confirmed. RSI: {row['RSI']}.")
                st.link_button(f"Analyze Chart 📈", row['Chart'], use_container_width=True)
    else:
        st.warning("No Bullish Support setups found.")
