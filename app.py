...
with st.expander("📝 View SEBI Compliance & Risk Terms", expanded=True):
    ...
    <div style="background-color: rgba(255, 193, 7, 0.05); padding:15px; border-radius:12px; border:1px solid rgba(255, 193, 7, 0.3);">
        ...
        <p style="color:#CCCCCC; font-size:0.95em;">
            <b>ArthaSutra</b> is an educational research algorithm. We are <b>NOT SEBI registered</b>. 
            All signals are mathematical derivations. <b>Trading involves risk.</b> Consult an advisor before investing.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    for i, ticker in enumerate(NIFTY_200):
        try:
            ...
            if d['Close'] > d['SMA_44'] and d['SMA_44'] > d['SMA_200'] and d['Close'] > d['Open']:
                ...
                analysis = get_deep_analysis(ticker, d, status, jackpot_hit, v_ratio)
...
with col_input:
    ...
    run_btn = st.button('🚀 Execute Deep Scan', use_container_width=True)
...
if run_btn:
    ...
    if not df.empty:
        ...
        st.write(f"### 📊 Institutional Report: {adjusted_date}")
        ...
        with col_dl:
            ...
            st.download_button("📂 Export Audit Data (CSV)", data=csv, file_name=f"ArthaSutra_{adjusted_date}.csv", mime='text/csv')

        st.divider()
        st.write("### 🔍 Live Signals Tracker")
        st.dataframe(df.drop(columns=['Deep_Reasoning', 'Jackpot']), use_container_width=True, hide_index=True)
        
        st.divider()
        st.write("### 💡 Comprehensive Post-Mortem Analysis")
