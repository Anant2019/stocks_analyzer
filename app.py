...
st.set_page_config(page_title="Final Stable Backtester", layout="wide")
...
st.title("🎯 Triple Bullish: Buffer Entry Dashboard")
target_date = st.date_input("Kounsi date analyze karni hai?", datetime.now() - timedelta(days=15))
    ...
    for i, ticker in enumerate(NIFTY_200):
        try:
            ...
            if c > s44 and s44 > s200 and c > o:
                # BUFFER ENTRY (+2 points as requested)
                entry_price = c + 2
                risk = entry_price - l
                ...
                if risk > 0:
                    ...
                    results.append({
                        ...
                        "Result": outcome,
                        "Days": days,
                        "Entry (Buffer)": round(entry_price, 2),
                        "T1": round(t1, 2),
                        "T2": round(t2, 2)
                    })
        except: continue
