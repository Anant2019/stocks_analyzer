...
st.markdown("""
    ...
    /* Custom Card Style for Mobile Feel */
    .report-card {
        background: #1A1C23;
        padding: 20px;
        border-radius: 15px;
...
with st.expander("📝 IMPORTANT: SEBI Non-Registration & Risk Warning", expanded=True):
    ...
    <div style="background-color: rgba(255, 193, 7, 0.05); padding:15px; border-radius:12px; border:1px solid rgba(255, 193, 7, 0.3);">
        ...
        <p style="color:#CCCCCC; font-size:0.95em;">
            ...
            Signals are mathematical derivations. <b>Trading involves high risk.</b> Equity trading is subject to market risks.
        </p>
    </div>
    """, unsafe_allow_html=True)
...
def get_technical_audit(d, status, v_ratio, is_blue, t1, t2):
    rsi = round(d['RSI'], 1)
        ...
        try:
            ...
            if d['Close'] > d['SMA_44'] and d['SMA_44'] > d['SMA_200'] and d['Close'] > d['Open']:
                ...
                results.append({
                    ...
                    "Audit": get_technical_audit(d, status, v_ratio, is_blue, t1, t2),
